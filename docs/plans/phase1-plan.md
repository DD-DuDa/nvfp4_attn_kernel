...
- **AC-4: FP4-Q numerical gate is added and exact.**
  - Positive: quantize a BF16 query once, feed both paths, and assert output byte equality across pure FP4, residual, and representative MHA/GQA/MQA cases; `tests/kernel` includes these tests.
...
- Can use quantizer-native `query_fp4` and `query_scales`, an internal helper to validate/prepare both contracts, and a minimal placeholder BF16 tensor only if the compiled core statically requires it and it is not allocated per call.
...
4. Add exact FP4-Q tests including residual and head configurations.
