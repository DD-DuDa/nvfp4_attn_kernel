...
# Vendored and namespace-adapted from flash-attn-4 4.0.0b23 for the standalone decode kernel.
...
from ._fa4 import utils
from ._fa4.combine_utils import assume_tensor_aligned
from ._fa4.seqlen_info import SeqlenInfo
...
def flash_attn_combine(
    ...
    """Combine fixed-shape split decode partials with the private CuTe kernel."""
    import torch
    from cutlass.cute.runtime import from_dlpack

    if out_dtype is None:
        out_dtype = out_partial.dtype
    num_splits, batch, seqlen, heads, head_dim = out_partial.shape
    if out is None:
        out = torch.empty(
            batch, seqlen, heads, head_dim,
            dtype=out_dtype, device=out_partial.device,
        )
    lse = (
        torch.empty(
            batch, seqlen, heads,
            dtype=torch.float32, device=out_partial.device,
        )
        if return_lse
        else None
    )
    log_max_splits = max(math.ceil(math.log2(num_splits)), 5)
    operation = FlashAttentionForwardCombine(
        cutlass.BFloat16 if out_dtype is torch.bfloat16 else cutlass.Float32,
        cutlass.Float32,
        head_dim,
        tile_m=8,
        k_block_size=128,
    ...
    key = (out_dtype, head_dim, log_max_splits, lse is not None)
    compiled = flash_attn_combine.compile_cache.get(key)
    def to_cute(tensor, leading_dim):
        if tensor is None:
            return None
        return from_dlpack(
    ...
    cute_args = (
        to_cute(out_partial, 4),
        to_cute(lse_partial, 3),
        to_cute(out, 3),
        to_cute(lse, 2),
        None,
        None,
...
flash_attn_combine.compile_cache = {}
