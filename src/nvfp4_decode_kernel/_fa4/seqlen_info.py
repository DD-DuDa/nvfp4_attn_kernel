...
class SeqlenInfoQK:
    ...
    def offset_batch_O_via_indices(
        self,
        mO: cute.Tensor,
        batch_idx: Int32,
        mOutIndices: cute.Tensor,
        dim: int,
    ) -> cute.Tensor:
        """Substitute for `offset_batch_Q` when output rows are indirect.

        Slice mO along `dim` at row
        `mOutIndices[batch_idx]`, producing the same shape
        `offset_batch_Q(..., dim)` would on the non-varlen path.
        mOutIndices[batch_idx] is the per-batch destination row in the flat
        global output (NOT a CDF — arbitrary positions per decode slot).
        """
        offset = mOutIndices[batch_idx]
        idx = (None,) * dim + (offset,) + (None,) * (cute.rank(mO) - 1 - dim)
        return mO[idx]
